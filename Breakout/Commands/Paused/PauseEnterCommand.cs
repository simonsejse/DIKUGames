    ...
    public void Execute() {
        ...
        switch (_menu.GetActiveMenuItem()) {
            case 0:
                ...
                @event = _gameEventFactory.CreateGameEvent(GameEventType.GameStateEvent, 
                    "CHANGE_STATE", Enum.GetName(GameState.Running) ?? "Running");
                break;
            case 1:
                @event = _gameEventFactory.CreateGameEvent(GameEventType.GameStateEvent, 
                    "CHANGE_STATE", Enum.GetName(GameState.Menu) ?? "Menu");
                break;
            default: 
